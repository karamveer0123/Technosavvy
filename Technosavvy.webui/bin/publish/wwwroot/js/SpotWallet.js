$('#sportWalletTable-main').simplePagination({
    items_per_page: 10,
...
function depositHand() {
    var Alloval = $('#id_sport .dyCur').attr('data-oval');
    if (Alloval == 0) {
...
function replaceTotal(data) {
    ...
    var Alloval = $('#id_sport .dyCur').text();
    if (Alloval == '$NaN') {
        $('#id_sport small').replaceWith('<small class="dyCur" data-dval="6" data-oval="' + AllTotal + '">' + AllTotal + '</small>');
    }
}
