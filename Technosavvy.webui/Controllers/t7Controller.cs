...
public class t7Controller : Controller
{
    ...
    public t7Controller(ILogger<HomeController> logger, IOptions<SmtpConfig> smtp, IConfiguration configuration, IHttpContextAccessor accessor, IDataProtectionProvider provider, ITempDataDictionaryFactory tddf)
    {
        _logger = logger;
        _smtp = smtp;
    ...
    {
        return View("deposit/deposit-fiat-upi");
    }
    [HttpGet]
    [ActionName("deposit-fiat")]
    public async Task<IActionResult> depositfiat()
    {
        return View("deposit/deposit-fiat");
    }
    [HttpGet]
    [ActionName("deposit")]
