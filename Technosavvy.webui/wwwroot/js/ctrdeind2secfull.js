...
function doStartup() {
    ...
    var txt = 'Available ' + GetFormatCountMBTQ(_qa, __qf) + ' ' + __q;
    var txtb = 'Available ' + GetFormatCountMBTQ(_ba, __bf) + ' ' + __b;
...
function updateLP(data) {
    if (lastPrice <= data.tradePrice) {
        var txt = '<div class="price-number green"> ' + GetFormatedVal(data.tradePrice, 2) + ' <i class="fa fa-long-arrow-up "></i> </div><div class="sub-number dyCur" data-oval="'+data.tradePrice+'">' + GetFormatedCurrency(data.tradePrice) + ' </div>';
        ...
        var txt = '<div class="price-number red"> ' + GetFormatedVal(data.tradePrice, 2) + ' <i class="fa fa-long-arrow-down "></i> </div><div class="sub-number dyCur" data-oval="' + data.tradePrice +'">' + GetFormatedCurrency(data.tradePrice) + ' </div>';
...
function updateTradeList(data) {
    if (!IsTradeFirst) {
        var txt = '<ul class="price-ul stickyTop"><li>Price (' + __q + ') <span> </span></li><li>Amount (' + __b + ') <span> </span></li><li>Time </li></ul>';
        $('#tradetab77').html(txt);
        IsTradeFirst = true;
    }
    var txt = '';
    ...
    if (lp.tradePrice > data.tradePrice) {
        txt = '<ul class="row-div" ><li class="red">' + GetFormatedValWithSep(data.tradePrice, __qf) + ' </li><li> ' + GetFormatedVal(data.tradeVolumn, __bf) + ' </li><li> ' + moment.utc(data.dateTimeUTC.toString()).local().format('hh:mm') + ' </li></ul>';
    }
    else {
...
function updateTMS(data) {
    ...
    var txt = '<div class="number-div"><ul><li> 24h <br> Change <span class="css44-up1 ' + clsrg + '"> ' + GetPercentFormat(data.change, __qf) + '</span>  </li><li> 24h<br> High <span> ' + GetFormatedVal(data.highest24H, __qf) + ' </span></li><li> 24h<br> Low <span> ' + GetFormatedVal(data.lowest24H, __qf) + ' </span> </li><li> 24h<br> Volume(' + data.base + ') <span> ' + GetFormatedValWithSep(data.volume, 2) + ' </span> </li><li> 24h <br>Volume(' + data.quote + ') <span> ' + GetFormatedValWithSep(data.trade24Hr, 2) + ' </span> </li></ul></div>';
    $('#mTop24h').html(txt);
